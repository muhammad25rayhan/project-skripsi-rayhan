...
class AnswerController extends Controller
{
    ...
    public function koreksiSoal(Request $request, $id_soal, $id_participan_session)
    {
        ...
        foreach ($soal as $item) {
            if ($item->answer) {
                if (!is_null($item->choice_1)) {
                    switch ($item->answer) {
                        case 'a':
                            $total_bobot += $item->weight_1;
                            break;
                        case 'b':
                            $total_bobot += $item->weight_2;
                            break;
                        case 'c':
                            $total_bobot += $item->weight_3;
                            break;
                        case 'd':
                            $total_bobot += $item->weight_4;
                            break;
                        case 'e':
                            $total_bobot += $item->weight_5;
                            break;
                        default:
                            # code...
                            break;
                    ...
                    if ($item->status == 'correct') {
                        $total_bobot += $item->weight_1; // Adjust according to your weight field for essay
                    }
                }
            }
        }
        return view('pages.answer.koreksi', compact('page', 'soal', 'benar', 'salah', 'banyak_soal', 'kosong', 'participant', 'data', 'total_bobot'));
    }
