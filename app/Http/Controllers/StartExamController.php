...
class StartExamController extends Controller
{
        ...
        if (Auth::user()->hasRole('siswa')) {
            ...
            $cekujian = Participant::join('participant_sessions as ps', 'participants.id', 'ps.participant_id')
                ...
                ->select(
                    ...
                    'class_rooms.name',
                    'exams.kode_ujian',
                    'participants.shcool_name',
                    'exams.description',
                    'ps.id as id_participant_sessions',
                    'ps.status'
                )
    ...
    public function finishExam(Request $request)
    {
        ...
        if (!$participant_session || $participant_session->status == 'finished') {
            return response()->json([
                'success' => false,
                'message' => 'Ujian tidak ditemukan atau sudah selesai!'
            ]);
        }
        ...
        try {
            $total_weight = 0;
            $pilgan_count = isset($data['pilgan']) ? count($data['pilgan']) : 0;

            if ($pilgan_count > 0) {
                foreach ($data['pilgan'] as $item) {
                    $soal = DetailQuestion::findOrFail($item['id']);
                    $answer = isset($item['answer']) ? $item['answer'] : '';
                    $status = ($soal->key == $answer && $answer !== '') ? 'correct' : ($answer !== '' ? 'wrong' : '');

                    // Determine the weight based on the selected answer
                    $weight = 0;
                    if ($answer === 'a') {
                        $weight = $soal->weight_1 ?? 0;
                    } elseif ($answer === 'b') {
                        $weight = $soal->weight_2 ?? 0;
                    } elseif ($answer === 'c') {
                        $weight = $soal->weight_3 ?? 0;
                    } elseif ($answer === 'd') {
                        $weight = $soal->weight_4 ?? 0;
                    } elseif ($answer === 'e') {
                        $weight = $soal->weight_5 ?? 0;
                    }
                ...
                // Store the raw sum of weights as nilai_pilgan
                $nilai_pilgan = $total_weight;
                ...
                Nilai::create([
                    ...
                    'nilai_pilgan' => $nilai_pilgan,
                    'nilai_rata_rata' => $nilai_pilgan, // Update later if essay scores are included
                ]);
            }
