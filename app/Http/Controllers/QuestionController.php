    ...
    public function store(Request $request)
    {
        if ($request->file('import')) {
            $this->validate($request, [
                ...
                'major_id' => 'required',
                'import' => 'required|mimes:xlsx,csv|max:50000'
            ], [
                'user_id.required' => 'Guru tidak boleh kosong!',
                'type.required' => 'Type tidak boleh kosong!',
                'type.in' => 'Type harus Pilihan Ganda atau Essay!',
                ...
                'import.mimes' => 'File harus berupa Excel (xlsx, csv)!',
                'import.max' => 'File tidak boleh lebih dari 50 MB!',
            ]);
        } else {
            $rules = [
                'user_id' => 'required',
                ...
                'data.*.image' => 'nullable|image|mimes:jpeg,png,jpg,svg|max:2048',
            ];

            if ($request->type === 'pilgan') {
                $rules = array_merge($rules, [
                    ...
                    'data.*.weight_1' => 'required|numeric|min:0|max:5',
                    'data.*.weight_2' => 'required|numeric|min:0|max:5',
                    'data.*.weight_3' => 'required|numeric|min:0|max:5',
                    'data.*.weight_4' => 'required|numeric|min:0|max:5',
                    'data.*.weight_5' => 'required|numeric|min:0|max:5',
                ]);
            }
            ...
            $this->validate($request, $rules, [
                ...
                'data.*.weight_1.required' => 'Bobot Pilihan 1 tidak boleh kosong!',
                'data.*.weight_1.numeric' => 'Bobot Pilihan 1 harus berupa angka!',
                'data.*.weight_1.min' => 'Bobot Pilihan 1 harus minimal 0!',
                'data.*.weight_1.max' => 'Bobot Pilihan 1 harus maksimal 1!',
                'data.*.weight_2.required' => 'Bobot Pilihan 2 tidak boleh kosong!',
                'data.*.weight_2.numeric' => 'Bobot Pilihan 2 harus berupa angka!',
                'data.*.weight_2.min' => 'Bobot Pilihan 2 harus minimal 0!',
                'data.*.weight_2.max' => 'Bobot Pilihan 2 harus maksimal 1!',
                'data.*.weight_3.required' => 'Bobot Pilihan 3 tidak boleh kosong!',
                'data.*.weight_3.numeric' => 'Bobot Pilihan 3 harus berupa angka!',
                'data.*.weight_3.min' => 'Bobot Pilihan 3 harus minimal 0!',
                'data.*.weight_3.max' => 'Bobot Pilihan 3 harus maksimal 1!',
                'data.*.weight_4.required' => 'Bobot Pilihan 4 tidak boleh kosong!',
                'data.*.weight_4.numeric' => 'Bobot Pilihan 4 harus berupa angka!',
                'data.*.weight_4.min' => 'Bobot Pilihan 4 harus minimal 0!',
                'data.*.weight_4.max' => 'Bobot Pilihan 4 harus maksimal 1!',
                'data.*.weight_5.required' => 'Bobot Pilihan 5 tidak boleh kosong!',
                'data.*.weight_5.numeric' => 'Bobot Pilihan 5 harus berupa angka!',
                'data.*.weight_5.min' => 'Bobot Pilihan 5 harus minimal 0!',
                'data.*.weight_5.max' => 'Bobot Pilihan 5 harus maksimal 1!',
                'data.*.key.required' => 'Kunci Jawaban tidak boleh kosong!',
                // 'data.*.key.in' => 'Kunci Jawaban harus a, b, c, d, atau e!',
        ...
        try {
            ...
            if ($request->file('import')) {
                ...
                foreach ($data['data'] as $value) {
                    ...
                    if (!empty($value['image'])) {
                        ...
                        Storage::makeDirectory('public/images/' . $id . '/' . $data['type']);
                        $file->storeAs('public/images/' . $id . '/' . $data['type'], $file_name);
                        $value['image'] = $file_name;
                    }
                    DetailQuestion::create($value);
                }
            }
            ...
            return response()->json([
                'success' => false,
                'message' => 'Question creation failed: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    public function createDetailQuestion(Request $request, $id)
    {
        ...
        if ($question->type == 'essay') {
            ...
            $this->validate($request, [
                ...
                'data.*.choice_4' => 'required',
                'data.*.choice_5' => 'required',
                'data.*.weight_1' => 'required|numeric|min:0|max:1',
                'data.*.weight_2' => 'required|numeric|min:0|max:1',
                'data.*.weight_3' => 'required|numeric|min:0|max:1',
                'data.*.weight_4' => 'required|numeric|min:0|max:1',
                'data.*.weight_5' => 'required|numeric|min:0|max:1',
                'data.*.image' => 'image|mimes:jpeg,png,jpg,svg|max:2048|nullable',
            ], [
                'data.*.question.required' => 'Soal tidak boleh kosong !',
                'data.*.choice_1.required' => 'Pilihan 1 tidak boleh kosong !',
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'Detail Question created successfully!'
            ]);
        } catch (Exception $e) {
            DB::rollback();
            return response()->json([
                'success' => false,
                'message' => 'Detail Question creation failed: ' . $e->getMessage()
            ]);
        }
    }
    ...
    public function updateDetailQuestion(Request $request, $id)
    {
        ...
        if ($question->type == 'pilgan') {
            $rules = array_merge($rules, [
                ...
                'choice_4' => 'required',
                'choice_5' => 'required',
                'weight_1' => 'required|numeric|min:0|max:5',
                'weight_2' => 'required|numeric|min:0|max:5',
                'weight_3' => 'required|numeric|min:0|max:5',
                'weight_4' => 'required|numeric|min:0|max:5',
                'weight_5' => 'required|numeric|min:0|max:5',
            ]);
        }
        ...
        return response()->json([
            'success' => true,
            'message' => 'Detail Question updated successfully!'
        ]);
    }
